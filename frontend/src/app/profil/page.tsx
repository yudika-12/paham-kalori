          ...
          <div className="mt-4 space-y-4">
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200/70 bg-white p-4 shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                ...
                {profile?.goal ? (
                  <span className="mt-0.5 inline-block rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700">
...
function ListRow({
  ...
  const inner = (
    <>
      <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${danger ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-700"}`}>
        {icon}
      </span>
      <span className={`flex-1 text-[13px] font-semibold ${danger ? "text-red-600" : "text-slate-800"}`}>{label}</span>
      {value ? <span className="text-[12px] font-bold text-slate-900">{value}</span> : null}
      {badge ? (
        <span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ color: badge.color, background: badge.bg }}>
          {badge.label}
        </span>
      ) : null}
      {chevron ? (
  ...
  const cls = `flex w-full items-center gap-2.5 bg-white px-3.5 py-2.5 text-left transition ${divide ? "border-t border-slate-100" : ""} ${danger ? "hover:bg-red-50/50" : onClick ? "hover:bg-slate-50" : ""}`;
  return onClick ? (
